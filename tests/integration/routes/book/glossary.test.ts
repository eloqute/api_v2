...
describe("GET /book/{publicationURL}/glossary", () => {
  beforeAll(setup);
  afterAll(teardown);

  describe("When no user is logged in", () => {
    it("returns a 401 error", async () => {
      const res = await request(app).get("/book/phenomenology-of-spirit-analysis/glossary").send();
  ...
  describe("When a user is logged in", () => {
    ...
    describe("When a book exists with the given Publication URL", () => {
      it("returns the glossary for the given book", async () => {
        const res = await ses!.get("/book/phenomenology-of-spirit-analysis/glossary").send();
        expect(res.status).toEqual(200);
        expect(res).toSatisfyApiSpec();
        expect(res.body.length).toBeGreaterThan(0);
        res.body.forEach(
          (item : object) => expect(item).toSatisfySchemaInApiSpec("GlossaryItem")
        );
      });
    });
    ...
    describe("When no book exists with the given Publiaction URL", () => {
      it("returns a 404 error", async () => {
        const res = await ses!.get("/book/doesnt-exist/glossary").send();
